...
export default function Family() {
  ...
  if (!activeFamily) {
    return (
      <div className="w-full max-w-2xl mx-auto py-16 text-center space-y-4">
        ...
        <h3 className="text-xl font-bold text-white">No Active Family Circle</h3>
  ...
  const openAccessModal = (member) => {
    ...
    // Backend currently doesn't expose canViewLocationsOf per member,
    // so this starts empty — admin explicitly sets access each time.
    setSelectedIds([]);
  };
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#e5e2e1] text-sm truncate">
                        {member.name}
                        {isSelf && (
                          <span className="text-[#8c90a0] font-normal"> (You)</span>
                        )}
                      </span>
                      <span
