...
export default function Dashboard() {
  ...
  return (
    <div className="bg-[#131313] text-[#e5e2e1] font-sans antialiased overflow-hidden min-h-screen selection:bg-[#b0c6ff]/30 [font-family:'Geist',sans-serif]">
      {/* ================= TOP NAVBAR ================= */}
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-5 h-16 bg-[#131313]/70 backdrop-blur-xl border-b border-white/5 shadow-sm">
        <div className="flex items-center gap-8">
          <span className="text-[48px] font-bold tracking-tighter text-[#b0c6ff] leading-[1.1] [letter-spacing:-0.02em]">
            SENTRY
          </span>
          <nav className="hidden md:flex gap-6 items-center">
            {["Dashboard", "Live Map", "Activity"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`pb-1 text-sm font-body-sm transition-all duration-200 active:scale-95 ${
                  activeTab === tab
                    ? "text-[#b0c6ff] font-bold border-b-2 border-[#b0c6ff]"
                    : "text-[#c2c6d7] font-medium hover:text-[#b0c6ff]"
                }`}
              >
                {tab}
              </button>
        ...
        <div className="flex items-center gap-4">
          <button className="p-2 rounded-full hover:bg-white/5 transition-colors active:scale-95 flex items-center">
            <span className="material-symbols-outlined text-[#c2c6d7]">
              notifications
            </span>
          </button>
          <div className="h-8 w-8 rounded-full bg-[#558dff] flex items-center justify-center overflow-hidden border border-white/10 active:scale-95 cursor-pointer">
            <img
              className="w-full h-full object-cover"
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuDL8WQdMqF_ek_4xseR3UtdO2reGP0zNgQ_Df5GgdxAVICJeQIwacaZ_7i1A2DmrdKSzk6GocyH4-x9CbF1IuRns2XA2evv2nOdZdm4wSUF9pIZHNGO5iStMktF75dAQ0d_tdp5qt6B2gqPxeY8cpeO9yNbsciAGeu3H9Do_D2kyR59wgsaqhA_D3_MyJTFgcC8GYFHR1QpT2fIsnRdD6_pwcDlDNKx4lSwLwWjYYyZZQ5-pmq2sM2nTOiTPJIZox2Kk6FumXTeABgn"
              alt="User profile summary view portrait"
            />
          </div>
        </div>
      </header>
      ...
      <aside className="hidden lg:flex flex-col h-full fixed left-0 top-0 py-10 px-6 w-64 bg-[#201f1f] border-r border-white/5 shadow-2xl z-40 pt-24">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-8 pt-2 bg-[#201f1f]/90 backdrop-blur-2xl border-t border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.5)] rounded-t-2xl">
        <button className="flex flex-col items-center justify-center text-[#4edea3] bg-[#00a572]/20 rounded-full p-2.5 active:scale-90 transition-all">
          <span
            className="material-symbols-outlined"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
        ...
        <button className="flex flex-col items-center justify-center text-[#c2c6d7] hover:text-[#e5e2e1] active:scale-90 transition-all p-2">
          <span className="material-symbols-outlined">location_on</span>
          <span className="text-[11px] font-bold tracking-wide mt-1">Map</span>
        </button>
        <button className="flex flex-col items-center justify-center text-[#c2c6d7] hover:text-[#e5e2e1] active:scale-90 transition-all p-2">
          <span className="material-symbols-outlined">person</span>
          <span className="text-[11px] font-bold tracking-wide mt-1">
            Profile
          </span>
        </button>
      </nav>
